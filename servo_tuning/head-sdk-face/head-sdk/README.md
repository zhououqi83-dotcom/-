...
在SDK目录下运行以下命令，以开发模式安装SDK：

```bash
cd head-sdk-headbase
pip install -e .
```

这将以可编辑模式安装SDK，任何对源代码的修改都会立即生效，无需重新安装。
...
# 或者用 uv / rye / pdm 等工具也可以，但这里用最基础的
# 一次性生成 sdist（.tar.gz） + wheel（.whl）两种格式，最推荐
python -m build
cd dist
pip install head_sdk-0.1.0-py3-none-any.whl
# 一定要先安装
...
如果遇到`ModuleNotFoundError: No module named 'head_sdk'`错误，请确保已经按照上述方法安装了SDK。
