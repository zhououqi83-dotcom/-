...
| 五个维度打分 | 脸的大小、多久没被看过、跟声源方向是否吻合、是否面朝机器人、是否在说话 |
...
**还没接的**:语义判定"现在在跟谁聊天"。`PersonRegistry.set_chat_target`/
`Person.is_chat_target` 这套底层接口还在,留给以后接 `nlu_intent` + 对话
管理器用,但**已经不参与打分**——之前"聊天对象"是信号之一,现在被
从打分公式里去掉了,调用这个接口目前对注视决策没有任何影响。
...
总分 = (w_size·size + w_novelty·novelty + w_sound·sound
       + w_facing·facing + w_speaking·speaking) / 权重总和
```
...
(比如没声音时 `sound=0`,乘法会让"没说话但脸很大"的人也变成
