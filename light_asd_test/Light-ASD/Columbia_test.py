		...
		for duration in durationSet:
			...
			with torch.no_grad():
				for i in range(batchSize):
					inputA = torch.FloatTensor(audioFeature[i * duration * 100:(i+1) * duration * 100,:]).unsqueeze(0).to('cuda' if torch.cuda.is_available() else 'cpu')
					inputV = torch.FloatTensor(videoFeature[i * duration * 25: (i+1) * duration * 25,:,:]).unsqueeze(0).to('cuda' if torch.cuda.is_available() else 'cpu')
